...
## Istio Security (mTLS + AuthorizationPolicy)

Istio manifests are in `k8s/istio/` and include:
- ServiceAccounts for each workload.
- `PeerAuthentication` with **STRICT mTLS**.
- `DestinationRule` objects using `ISTIO_MUTUAL` for service-to-service traffic.
- `AuthorizationPolicy` rules to allow only:
  - frontend -> backend
  - backend -> database
...
1. Confirm frontend can still call backend through the app flow.
2. From a temporary pod that is **not** authorized as frontend/backend, test direct access (should be denied):

```bash
kubectl run istio-test --image=nicolaka/netshoot --restart=Never -it --rm -- /bin/bash
curl -v http://backend:5001/api/data
curl -v database:5432
```

If needed, remove Istio policies:
